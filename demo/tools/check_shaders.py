...
the vertex quad and the card itself -- the lens, the print, the focus spot's
coverage and the ridge. Two copies of the same text drift, quietly, because a
demo that renders a *plausible* lenticular card looks exactly like one that
renders the right one. The page's whole claim is that it runs the plugin's own
shader, so the claim needs something enforcing it. `lntest` drives the real
...
other backslash on the JS side is rejected*. Lenticular's shaders carry neither a
backtick nor a backslash today, so the decoder is idle; if one ever appears in
the C++, this is where it is accounted for.

------------------------------------------------------------------- what it cannot

Nothing here checks the *ported* half: the Controls.cpp conversions, Lens.h's
constants and the uniform arithmetic of ProcessOpenGL in `demo/plugin.js` are
a hand translation. demo/README.md records the one time it was compared with
`lntest --pipe`; after that only a reader can tell whether they still agree.
...
# JS constant, C++ file, C++ symbol. Lenticular has one pass: the quad and the card.
SHADERS = [
    ("VERTEX", "source/Shaders.cpp", "kVertexShader"),
    ("LENTICULAR", "source/Shaders.cpp", "kLenticularShader"),
]
